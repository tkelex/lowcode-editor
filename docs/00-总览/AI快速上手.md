...
4. 如果有多个窗口并行工作，读同级资料仓库 `../lowcode-editor-notes/docs/10-协作/并行任务看板.md`。
...
```text
请先读取 AGENTS.md、docs/00-总览/AI快速上手.md、../lowcode-editor-notes/docs/10-协作/并行任务看板.md，
...
Next.js 发布页运行时
  服务端读取公开发布快照、生成 metadata、缓存公开页并水合低代码运行态
...
Next 公开访问 -> /publish/[publicId] -> GET public snapshot -> Preview(allowCustomJS=false)
```
...
| Next.js 公开发布页 | `apps/publisher-next/app/publish/[publicId]/page.tsx`、`apps/publisher-next/src/published-pages/*`、`src/editor/runtime/Preview/index.tsx`、`published-page-revalidate.service.ts` |
...
- 不要让公开发布页执行自定义 JS。
- 不要把编辑器主应用整体迁移到 Next.js；Next.js 只承担公开发布页运行时。
- 不要只改前端权限隐藏，后端必须校验权限。
...
文档任务：说明精简了哪些入口、哪些内容被迁移到 `../lowcode-editor-notes`。
