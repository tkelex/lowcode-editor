...
import type { EditorPage, PageSchema, PageVersion } from '../types';
...
export async function publishPage(pageId: number) {
  const { data } = await http.post<EditorPage>(`/pages/${pageId}/publish`);
  return data;
}

export async function unpublishPage(pageId: number) {
  const { data } = await http.post<EditorPage>(`/pages/${pageId}/unpublish`);
  return data;
}
